		...
		<div class="space-y-8 divide-y divide-gray-200 sm:space-y-5">
			<div class="space-y-6 sm:space-y-5">
				...
				<div class="space-y-6 sm:space-y-5">
					...
					<div class="sm:grid sm:grid-cols-4 sm:items-start sm:gap-4 sm:border-t sm:border-gray-200 sm:pt-5">
						<label for="cron" class="block text-md font-medium text-gray-700 sm:mt-px sm:pt-2"> Cron </label>
						<div class="mt-1 sm:col-span-3 sm:mt-0">
							<input
								type="text"
								name="cron"
			...
			{#each data.events as event}
				<div class="border border-gray-400 rounded-md">
					...
					<div class="p-2 bg-white text-gray-700 rounded-b-md">
						<span class="block mb-2 font-semibold">{event.subject}</span>
						<pre class="block text-sm">{event.body}</pre>
					</div>
				</div>
			{/each}
		</div>
