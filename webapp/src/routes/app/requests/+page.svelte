<script>
  import { Heading, Table, Button, Link } from "$lib/components"

  export let data
</script>

<svelte:head>
  <title>Requests | Scrapi</title>
</svelte:head>

<Heading title="Requests" subtitle="All Requests made using Scrapi" />
...
>
  <div slot="cell" let:value let:row let:header>
    ...
    {:else if header.key === "job.name"}
      <Link href={`/app/jobs/${row.job.id}`} showIcon={true}>
        {value}
      </Link>
